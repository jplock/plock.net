...
=> https://www.linkedin.com/pulse/s3-access-points-fun-profit-justin-plock S3 Access Points for fun and profit (LinkedIn)
...
=> https://docs.aws.amazon.com/AmazonS3/latest/dev/access-points.html S3 Access Points

were released at re:Invent 2019 as a way to enhance access to S3 buckets in your account. Each access point has its own IAM policy and can be attached to a VPC. Before access points, there wasn't a way to write an S3 VPC endpoint policy that said:

* Allow access to buckets owned by my account
* Allow access to specific public buckets

mostly because the S3 Amazon Resource Name (ARN) was "arn:aws:s3:::example-bucket" and there wasn't a way to restrict it by account. With access points, we can now limit access to a bucket from within a specific VPC in an account.

At work, we wanted to limit S3 access through an S3 VPC endpoint to an approved list of public and private buckets. However, having to continually update the S3 VPC endpoint policy, whenever a new private bucket was created, was becoming a maintenance burden.

To solve the problem, we added the below S3 bucket policy to our S3 buckets to delegate control to access points:
...
We then added an access point to the example-bucket called my-access-point and added this S3 access point policy:
...
The above policy allows object-level access to the account-owned S3 access points and to an approved list of public S3 buckets (for example, the SSM Agent and CloudFormation). Bucket operations would not be able to traverse the VPC endpoint and must be done either in the Console or through the API.
